...
- `happy-dom` (at least the version this repo pins) does not implement declarative-shadow-DOM parsing — setting `.innerHTML` to a string containing `<template shadowrootmode="open">` leaves it as an inert light-DOM `<template>` element rather than hoisting it into a real, attached shadow root, even though it does correctly upgrade the custom element tag to its registered class. This means `hydration_int_test.ts` (which runs under `happy-dom`) can only verify DOM-node-identity reuse, not that the server-rendered shadow *content* survives without a re-render.
- **Verified against a real browser, not just assumed (justjs#66, resolved):** `tooling/ssr/scm/main/src/tests/hydration_real_browser_test.ts` drives a real, locally-launched Chrome via `chromiumctl-cli` (a sibling Rust CDP tool, `../chromiumctl` — not Playwright; rejected justweb's own embedded `browser-sdk` too, since its CDP bridge hard-depends on a `jsc` binary this environment doesn't have). Confirmed: real Chrome hoists `<template shadowrootmode="open">` into a genuine, attached shadow root with the exact server-rendered content, and a client-side custom element class re-registering and upgrading (with the realistic `if (this.shadowRoot) return` guard) does not clobber it. The production claim `happy-dom` couldn't verify is now a tested fact, not an assumption. This test is skipped (not failed) when `chromiumctl-cli` isn't available locally — it's an external tool, not a workspace dependency.
...
- justjs#64 — `adaptCustomElementRegistry`'s "Illegal constructor" gap, discovered while building this ADR
- justjs#66 — the real-browser hydration verification this ADR's Consequences section now records as resolved
...
- `../chromiumctl` (`chromiumctl-cli`) — the sibling CDP tool `hydration_real_browser_test.ts` uses to verify against a real browser
