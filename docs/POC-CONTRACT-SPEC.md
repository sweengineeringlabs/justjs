...
**Status:** POC — v0.8 (proved end-to-end with a real justweb-generated component mounted into a real DOM, registry shape fully consolidated — see justjs#38/#39/#41/#49/#50 and justweb ADR-0006/ADR-0008/#52/#56/#57)
...
>
> **Correction note (v0.7):** built a real happy-dom-based integration test using a captured real justweb-generated component (not a hand-authored fixture) run through `adaptCustomElementRegistry` + `DefaultLifecycle` — a real custom element is now proven to actually mount, register, and receive prop updates in a live DOM, closing justjs#39's remaining "not yet a full live render" gap. Building that test caught a real, independent bug: `adaptCustomElementRegistry` was built (justjs#46) to return a `DefaultComponentRegistry` instance, but `RenderStep`/`UpdateStep` only ever read a separate, plain `ComponentRegistry` Record shape — `DefaultComponentRegistry` was (and still is, at this point) never actually consumed by the lifecycle. As an interim fix, the adapter was changed to return the Record shape directly instead (superseded in v0.8 below).
>
> **Correction note (v0.8, justjs#50 resolved):** rather than leave `DefaultComponentRegistry` as real, tested, exported dead code with no caller (a rediscovery of #38's original "two unreconciled registry designs" finding), `RenderStep`/`UpdateStep`/`DefaultLifecycle` were changed to consume `DefaultComponentRegistry` directly (`.get(tag, props)` instead of a bracket lookup) — the plain `ComponentRegistry` Record type had zero other consumers anywhere in the codebase, so it was removed entirely (from `api/registry.ts` and the SAF surface) rather than kept as a second, parallel shape. `adaptCustomElementRegistry` reverted to returning `DefaultComponentRegistry` (its original, correct shape once the actual bug — the lifecycle never calling `.get()` — was fixed at the true call site instead of routed around).
...
**`@justjs/application` side:** `DefaultComponentRegistry`'s own shape — lazy `(props?) => Component` factories via `.register(tag, factory)`, enforcing hyphenated tags — is a third, independent shape again. Bridging `COMPONENT_REGISTRY` to it is `@justjs/application`'s own adapter to write (ADR-0008 is explicit this isn't justweb's decision); see `adaptCustomElementRegistry` in `application/scm/main/src/core/registry/component_registry_adapter.ts` (justjs#46). **Resolved (justjs#50, v0.8):** `RenderStep`/`UpdateStep` now actually call `DefaultComponentRegistry.get()` — a real bug (found while building a real-DOM integration test) where the lifecycle only ever did a bracket lookup against a separate, now-removed plain `ComponentRegistry` Record type, meaning `DefaultComponentRegistry` was real, tested, exported code with no actual caller.
