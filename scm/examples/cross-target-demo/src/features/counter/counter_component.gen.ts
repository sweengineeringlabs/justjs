// AUTO-GENERATED — do not edit. Regenerate with: jsc dom counter_component.yaml
// Source: counter_component.yaml (version 1)

export class CounterBase extends HTMLElement {
  static readonly tagName = 'js-counter';
  ...
  private _bindElements(): void {
    if (!this.#content) {
      ...
      if (__el) {
        this.#content = __el;
        __el.setAttribute('data-ddas-id', 'cross-target-demo:counter:counter:content');
      }
    }
  }
...
if (typeof customElements === 'object' && customElements !== null && typeof customElements.define === 'function') {
  const __existing = customElements.get('js-counter');
  if (!__existing) {
    customElements.define('js-counter', CounterBase);
  } else if (__existing !== CounterBase) {
    const __msg = "[justweb] custom element 'js-counter' is already registered to a different class";
    const __strict: boolean = (globalThis as { __justwebRegistryStrict__?: boolean }).__justwebRegistryStrict__ === true;
    if (__strict) { throw new Error(__msg); }
