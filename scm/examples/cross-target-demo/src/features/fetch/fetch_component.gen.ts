// AUTO-GENERATED — do not edit. Regenerate with: jsc dom fetch_component.yaml
// Source: fetch_component.yaml (version 1)

export class FetchBase extends HTMLElement {
  static readonly tagName = 'js-fetch';
  ...
  private _bindElements(): void {
    if (!this.#content) {
      ...
      if (__el) {
        this.#content = __el;
        __el.setAttribute('data-ddas-id', 'cross-target-demo:fetch:fetch:content');
      }
    }
  }
...
if (typeof customElements === 'object' && customElements !== null && typeof customElements.define === 'function') {
  const __existing = customElements.get('js-fetch');
  if (!__existing) {
    customElements.define('js-fetch', FetchBase);
  } else if (__existing !== FetchBase) {
    const __msg = "[justweb] custom element 'js-fetch' is already registered to a different class";
    const __strict: boolean = (globalThis as { __justwebRegistryStrict__?: boolean }).__justwebRegistryStrict__ === true;
    if (__strict) { throw new Error(__msg); }
