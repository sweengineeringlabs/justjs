// fetch — native integration test (auto-generated)
// Source: styles/fetch_component.yaml (v1)
// Run:    justw test src/features/fetch/fetch_int_test.gen.ts --native
//
...
import { FetchBase } from './fetch_component.gen';
...
test('fetch_module_exports_FetchBase_class', () => {
  assert(typeof FetchBase === 'function', 'FetchBase must be exported as a class/constructor');
});

test('fetch_tag_name_is_a_valid_custom_element_name', () => {
  const tag: string = FetchBase.tagName;
  assert(typeof tag === 'string' && tag.length > 0, 'FetchBase.tagName must be a non-empty string');
