// AUTO-GENERATED — do not edit. Regenerate with: jsc dom login_component.yaml
// Source: login_component.yaml (version 1)

export class LoginBase extends HTMLElement {
  static readonly tagName = 'js-login';
  ...
  private _bindElements(): void {
    if (!this.#content) {
      ...
      if (__el) {
        this.#content = __el;
        __el.setAttribute('data-ddas-id', 'cross-target-demo:login:login:content');
      }
    }
  }
...
if (typeof customElements === 'object' && customElements !== null && typeof customElements.define === 'function') {
  const __existing = customElements.get('js-login');
  if (!__existing) {
    customElements.define('js-login', LoginBase);
  } else if (__existing !== LoginBase) {
    const __msg = "[justweb] custom element 'js-login' is already registered to a different class";
    const __strict: boolean = (globalThis as { __justwebRegistryStrict__?: boolean }).__justwebRegistryStrict__ === true;
    if (__strict) { throw new Error(__msg); }
