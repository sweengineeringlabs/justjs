// review — native integration test (auto-generated)
// Source: styles/review_component.yaml (v1)
// Run:    justw test src/features/review/review_int_test.gen.ts --native
//
...
import { ReviewBase } from './review_component.gen';
...
test('review_module_exports_ReviewBase_class', () => {
  assert(typeof ReviewBase === 'function', 'ReviewBase must be exported as a class/constructor');
});

test('review_tag_name_is_a_valid_custom_element_name', () => {
  const tag: string = ReviewBase.tagName;
  assert(typeof tag === 'string' && tag.length > 0, 'ReviewBase.tagName must be a non-empty string');
