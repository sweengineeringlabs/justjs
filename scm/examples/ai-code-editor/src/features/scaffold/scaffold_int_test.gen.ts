// scaffold — native integration test (auto-generated)
// Source: styles/scaffold_component.yaml (v1)
// Run:    justw test src/features/scaffold/scaffold_int_test.gen.ts --native
//
...
import { ScaffoldBase } from './scaffold_component.gen';
...
test('scaffold_module_exports_ScaffoldBase_class', () => {
  assert(typeof ScaffoldBase === 'function', 'ScaffoldBase must be exported as a class/constructor');
});

test('scaffold_tag_name_is_a_valid_custom_element_name', () => {
  const tag: string = ScaffoldBase.tagName;
  assert(typeof tag === 'string' && tag.length > 0, 'ScaffoldBase.tagName must be a non-empty string');
