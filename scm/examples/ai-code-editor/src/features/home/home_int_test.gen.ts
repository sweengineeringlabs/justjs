// home — native integration test (auto-generated)
// Source: styles/home_component.yaml (v1)
// Run:    justw test src/features/home/home_int_test.gen.ts --native
//
...
import { HomeBase } from './home_component.gen';
...
test('home_module_exports_HomeBase_class', () => {
  assert(typeof HomeBase === 'function', 'HomeBase must be exported as a class/constructor');
});

test('home_tag_name_is_a_valid_custom_element_name', () => {
  const tag: string = HomeBase.tagName;
  assert(typeof tag === 'string' && tag.length > 0, 'HomeBase.tagName must be a non-empty string');
