// workspace — native integration test (auto-generated)
// Source: styles/workspace_component.yaml (v1)
// Run:    justw test src/features/workspace/workspace_int_test.gen.ts --native
//
...
import { WorkspaceBase } from './workspace_component.gen';
...
test('workspace_module_exports_WorkspaceBase_class', () => {
  assert(typeof WorkspaceBase === 'function', 'WorkspaceBase must be exported as a class/constructor');
});

test('workspace_tag_name_is_a_valid_custom_element_name', () => {
  const tag: string = WorkspaceBase.tagName;
  assert(typeof tag === 'string' && tag.length > 0, 'WorkspaceBase.tagName must be a non-empty string');
