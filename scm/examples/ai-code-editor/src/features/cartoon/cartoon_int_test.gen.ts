// cartoon — native integration test (auto-generated)
// Source: styles/cartoon_component.yaml (v1)
// Run:    justw test src/features/cartoon/cartoon_int_test.gen.ts --native
//
...
import { CartoonBase } from './cartoon_component.gen';
...
test('cartoon_module_exports_CartoonBase_class', () => {
  assert(typeof CartoonBase === 'function', 'CartoonBase must be exported as a class/constructor');
});

test('cartoon_tag_name_is_a_valid_custom_element_name', () => {
  const tag: string = CartoonBase.tagName;
  assert(typeof tag === 'string' && tag.length > 0, 'CartoonBase.tagName must be a non-empty string');
