// AUTO-GENERATED — do not edit. Regenerate with: jsc dom form-demo_component.yaml
// Source: form-demo_component.yaml (version 1)

export class FormDemoBase extends HTMLElement {
  static readonly tagName = 'js-form-demo';
  ...
  private _bindElements(): void {
    if (!this.#content) {
      ...
      if (__el) {
        this.#content = __el;
        __el.setAttribute('data-ddas-id', 'hello-justjs:form-demo:form-demo:content');
      }
    }
  }
...
if (typeof customElements === 'object' && customElements !== null && typeof customElements.define === 'function') {
  const __existing = customElements.get('js-form-demo');
  if (!__existing) {
    customElements.define('js-form-demo', FormDemoBase);
  } else if (__existing !== FormDemoBase) {
    const __msg = "[justweb] custom element 'js-form-demo' is already registered to a different class";
    const __strict: boolean = (globalThis as { __justwebRegistryStrict__?: boolean }).__justwebRegistryStrict__ === true;
    if (__strict) { throw new Error(__msg); }
