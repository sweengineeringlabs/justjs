// AUTO-GENERATED — do not edit. Regenerate with: jsc dom fetch-demo_component.yaml
// Source: fetch-demo_component.yaml (version 1)

export class FetchDemoBase extends HTMLElement {
  static readonly tagName = 'js-fetch-demo';
  ...
  private _bindElements(): void {
    if (!this.#content) {
      const __el = this.querySelector('[data-part="content"]') as HTMLElement | null;
      if (__el) {
        this.#content = __el;
        __el.setAttribute('data-ddas-id', 'hello-justjs:fetch-demo:fetch-demo:content');
      }
    }
  }
...
if (typeof customElements === 'object' && customElements !== null && typeof customElements.define === 'function') {
  const __existing = customElements.get('js-fetch-demo');
  if (!__existing) {
    customElements.define('js-fetch-demo', FetchDemoBase);
  } else if (__existing !== FetchDemoBase) {
    const __msg = "[justweb] custom element 'js-fetch-demo' is already registered to a different class";
    const __strict: boolean = (globalThis as { __justwebRegistryStrict__?: boolean }).__justwebRegistryStrict__ === true;
    if (__strict) { throw new Error(__msg); }
