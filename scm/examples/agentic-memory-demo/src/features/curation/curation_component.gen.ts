// AUTO-GENERATED — do not edit. Regenerate with: jsc dom curation_component.yaml
// Source: curation_component.yaml (version 1)

export class CurationBase extends HTMLElement {
  static readonly tagName = 'js-curation';
  ...
  private _bindElements(): void {
    if (!this.#content) {
      ...
      if (__el) {
        this.#content = __el;
        __el.setAttribute('data-ddas-id', 'agentic-memory-demo:curation:curation:content');
      }
    }
  }
...
if (typeof customElements === 'object' && customElements !== null && typeof customElements.define === 'function') {
  const __existing = customElements.get('js-curation');
  if (!__existing) {
    customElements.define('js-curation', CurationBase);
  } else if (__existing !== CurationBase) {
    const __msg = "[justweb] custom element 'js-curation' is already registered to a different class";
    const __strict: boolean = (globalThis as { __justwebRegistryStrict__?: boolean }).__justwebRegistryStrict__ === true;
    if (__strict) { throw new Error(__msg); }
