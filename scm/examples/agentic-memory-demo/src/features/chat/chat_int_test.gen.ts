// chat — native integration test (auto-generated)
// Source: styles/chat_component.yaml (v1)
// Run:    justw test src/features/chat/chat_int_test.gen.ts --native
//
...
import { ChatBase } from './chat_component.gen';
...
test('chat_module_exports_ChatBase_class', () => {
  assert(typeof ChatBase === 'function', 'ChatBase must be exported as a class/constructor');
});

test('chat_tag_name_is_a_valid_custom_element_name', () => {
  const tag: string = ChatBase.tagName;
  assert(typeof tag === 'string' && tag.length > 0, 'ChatBase.tagName must be a non-empty string');
