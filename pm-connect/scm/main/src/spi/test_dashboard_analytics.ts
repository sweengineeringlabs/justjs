...
// - no network call, ever. Same reasoning as spi/linear.js et al: lives
// alongside real strategies rather than behind a build flag, and only
// activates if some catalog/UI explicitly requests strategy "testpm".
justjs.providers.register({
  concern: "dashboardAnalytics",
  strategy: "testpm",
  factory: (config?: AnalyticsProviderConfig) => new TestPmDashboardAnalyticsProvider(config ?? {}),
});
