...
// Same shape as @justjs/social-connect's own api/analytics.ts (justjs#137)
// - a free-form label + count rather than a fixed set of named fields,
// since a real SCM provider's own engagement vocabulary (open PRs,
// assigned issues, merge conflicts, review requests...) doesn't map onto
// one fixed schema. `items` backs the metric's own drill-down - always
...
// shape, not tied to ScmResource, since "trending" is a ranking over
// whatever a provider considers noteworthy right now (e.g. most-active
// repos), not the same list ScmConnectProvider.connect() already
// returns.
...
// whatever scmConnect already holds).
...
export interface DashboardAnalyticsProvider {
  ...
  // Real no-op, same boot()-contract reason ScmConnectProvider.weave()
