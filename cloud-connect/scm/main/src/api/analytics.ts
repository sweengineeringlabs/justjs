...
// Same shape as @justjs/social-connect's/@justjs/scm-connect's/
// @justjs/pm-connect's own api/analytics.ts (justjs#137/#139) - a
// free-form label + count rather than a fixed set of named fields,
// since a real cloud provider's own engagement vocabulary (running
// instances, failed deploys, billing alerts...) doesn't map onto one
// fixed schema. `items` backs the metric's own drill-down - always
...
// shape, not tied to CloudResource, since "trending" is a ranking over
// whatever a provider considers noteworthy right now (e.g. most-active
// deployments/instances), not the same list
// CloudConnectProvider.connect() already returns.
...
// whatever cloudConnect already holds).
